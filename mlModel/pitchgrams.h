...
{
public:
    PitchGramModel( int gramCount, std::string inModelName )
    {
    ...
    virtual void predict(const std::string& fileName ) override
    {
        int state = fileName2State(fileName);
        ...
        for (size_t i = 0; i < input.size() - m_gramCount; i++)
        {
           ...
           // auto scores = sortIndexes<double, NUMBER_OF_PEOPLE>(tempHolder);
            ...
            speakerResultList[state].push_back(  tempHolder );
        }
    }
    ...
    virtual void load() override
    {
        for ( int i = 0; i < NUMBER_OF_PEOPLE; i++)
        {
            std::unordered_map < size_t, double > tempMap;
